                    ...
                    try {
                        if (recipe.author_username != user.username){
                            ...
                            RecipePlaylist.find({recipes: {$in: [req.body.recipe_id]}}, function (err, recipePlaylists){
                                ...
                                User.find({fav_recipes: {$in: [req.body.recipe_id]}}, function (err, users){
                                    try {

                                        for (var i = 0; i < users.length; i++){
                                            var index = users[i].fav_recipes.indexOf(req.body.recipe_id)
                                            if (index > -1){
                                                users[i].recipes.splice(index, 1)
...
// Export start and stop server functions:
var server;
...
exports.stop = function(){
    if (server != undefined){
        console.log("Cookbook server stopped.");
        server = undefined;
    }
}
